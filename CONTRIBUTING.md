...
VirtualBox is the recommended provider, but the `Vagrantfile` also supports [libvirt](https://github.com/vagrant-libvirt/vagrant-libvirt)/QEMU. The `cloud-image/ubuntu-26.04` box ships both a VirtualBox and a libvirt image, so there is nothing to change on your side — just boot the machine with:

    $ vagrant up --provider=libvirt --provision

Your working copy is copied into the VM at `/vagrant` with Vagrant's `rsync` synced folder (a live 9p/VirtFS share is not used: the setup scripts `chmod` files in `/vagrant`, which 9p rejects). The sync is one-way (host → guest): after editing files on the host, run `vagrant rsync` — or `vagrant rsync-auto` to keep watching — to push the changes into the VM before re-provisioning.
